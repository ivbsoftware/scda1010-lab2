...
The dataset was loaded directly from the site \citep{WineDataset} using the R statement below. Note that column names were assigned as the dataset headers had long name making it inconvinient to present it in tables and charts. Corresponence of variable codes to long names is shown in the [previous section](#data-understanding).
...
\newpage
## Preview of the data
To pretty-print the first 20 rows of the dataset xtable \citep{R-xtable} library was used to generate Table \ref{table:dhead10}.

```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results='asis'}
library(xtable)
xtable(head(wines_red_data, n=20), 
  caption = "\\tt Red Wines Quality Dataset - first 20 rows", label = "table:dhead10")
```

## Distribution of target value in the dataset
As we mentione before, the target value QLT of the wine quality is not equaly distributed. The Figure \ref{fig:hist_qlt_rw} demonstrates the distribution. As we can see, dataset covers medium-quality wines with QLT between 5 and 7 well, low and high  quality wines represented poorly.
...
The following code calculates the default OLS model using all the indepedent variables. Results of the calculatiions presented below.
...
As we can see, Adjusted R-squared is pretty low at `r fit.sum["adj.r.squared"]`. In addition several varibles, like FA, CA, RS, and DEN
have shown to be not significant at p > 0.05. Let's remove them from the model and check if this helps increase the model reliabiliry.
...
As we can see, the reliability of the new model slightly inreased. We now have Adjusted R-squared at `r fit1.sum["adj.r.squared"]` comparing to previous `r fit.sum["adj.r.squared"]`

## Stepwise Regression
Stepwise Regression method allows to find the best regression model automatically. Code below searches for the best fit iterating through the combination of the independent variables to find the best fit. As we see from the algorithm output the new  model is identical to the one found the the previous section.
...
The code below generates a histogram of residuals \ref{fig:levr1_rw} for the OLS model hew have so far. From the chart we can say that the
resulting histogram is skewed. This should be addressed later.

```{r levr1_rw, fig.align="center", fig.cap="Histogram of Residual Errors", fig.height=3, fig.pos='h', fig.width=5, message=FALSE, warning=FALSE}
...
```{r resplt_rw, fig.align="center", fig.cap="Component + Resuduals Plot", fig.height=9, fig.width=6, message=FALSE, warning=FALSE}
library(car)
